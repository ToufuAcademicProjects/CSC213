...
//One thread per cell, one plock per game.
...
__global__ void solve_all_boards(board_t* original_board)
{

  int index = threadIdx.x; //Gets the cell number we want

  int square_starters[9] = {0,3,6,27,30,33,54,57,60}; //This is the top left number in each square region

  //Sets our board to the shared memory
  __shared__ board_t board; 
  board.cells[index] = original_board[blockIdx.x].cells[index];
 
  __syncthreads();

  //Gets the row/col/region of the index we're looking at
  int row = index/9;
  int col = index%9;
  int square = (col/3) + 3*(row/3);

  //Our continuing variable
  int contin = 1;

  //While we still are making progress, we keep going
  while(__syncthreads_count(contin) > 0)
    {
      //We set our continue variable to false, when we make progress we will set it to true
      contin = 0;  
      ...
      if(cell_to_digit(cell) == 0)
        {
          ...
          while(i<9)
            {
              //The cell we're comparing to is a single number, we want to change our cell number
              //Use bitwise operation to remove the potential value
              int val = cell_to_digit(board.cells[r+i]);
              if(val != 0)
                {
                  uint16_t temp = cell;
                  //Take our current cell and subtract the possibility of this cell's value.
                  cell &= ~(1<<val);
                  if(temp != cell)
                      contin = 1;
                }
          ...
          //We look through the columns
          int j = 0;
          while(j<9)
            {
              int val = cell_to_digit(board.cells[col+j*9]);
              if(val != 0)
                {
                  //The cell we're comparing to is a single number, we want to change our cell number
                  //Use bitwise operation to remove the potential value
          ...
          //We look through the square
          uint16_t start = square_starters[square];
          for(int a = 0; a<3; a++)
            {
              for(int b = 0; b<3; b++)
                {
                  int val = cell_to_digit(board.cells[start + a*9 + b]);
                  if(val != 0)
                    {
                      uint16_t temp = cell;
  ...
  //Updates our original board from the shared memory
  original_board[blockIdx.x].cells[index] = board.cells[index];
...
{
  //Creates our managed memory
  board_t* bods;
  cudaMallocManaged(&bods, num_boards*sizeof(board_t));
  //Moves our memory over to the managed memory
  memcpy(bods, boards,  num_boards*sizeof(board_t));
  //Calls the function on the GPU to solve everything
  solve_all_boards<<<num_boards,THREADS_PER_BLOCK>>>(bods);
  cudaDeviceSynchronize();
  //Moves our memory back to the original pointer
  memcpy(boards, bods,  num_boards*sizeof(board_t));
